...
def parse_args():
    parser = argparse.ArgumentParser(description="Train standard self-training baseline.")
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--labeled-csv", type=str, required=True)
    parser.add_argument("--pseudo-csv", type=str, required=True)
...
def main():
    ...
    pseudo_dataset = HAM10000Dataset(
    ...
    train_dataset = ConcatDataset([labeled_dataset, pseudo_dataset])
    ...
    print(f"Saved best self-training checkpoint to {checkpoint_path}")
